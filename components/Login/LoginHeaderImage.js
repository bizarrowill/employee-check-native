...
class LoginHeaderImage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.loginStatus == "fbchecking" &&
      nextProps.loginStatus == "fbloginfailed"
    ) {
  ...
  _renderImage(image) {
    if (
      this.props.keyboardflag == false ||
      this.props.loginStatus == "fbchecking" ||
      this.state.fbcheckingFinished
    ) {
      if (
        this.props.emailPwdBtnStr == "SignUp" ||
        ...
        image = (
          <Image
            ...
            source={require("./../../assets/images/backgroundLoginV2.png")}
          />
        );
        return image;
      }
      if (
        this.props.emailPwdBtnStr == "Profile" ||
        this.props.emailPwdBtnStr == "Reset"
      ) {
        return (
  ...
  render() {
    return <View>{this._renderImage()}</View>;
  }
}
